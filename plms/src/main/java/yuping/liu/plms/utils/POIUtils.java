...
public class POIUtils {
    ...
    public static ResponseEntity<byte[]> employee2Excel(List<Employee> list) {
        ...
        //文档管理员
        docInfo.setManager("yuping");
        //设置公司信息
        docInfo.setCompany("www.yuping.org");
        //4. 获取文档摘要信息
        SummaryInformation summInfo = workbook.getSummaryInformation();
        //文档标题
        summInfo.setTitle("员工信息表");
        //文档作者
        summInfo.setAuthor("yuping");
        // 文档备注
        summInfo.setComments("本文档由yuping提供");
        //5. 创建样式
        //创建标题行的样式
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            cell4.setCellStyle(dateCellStyle);//设置日期样式
       ...
       ByteArrayOutputStream baos = new ByteArrayOutputStream();//字节数组输出流在内存中创建一个字节数组缓冲区，所有发送到输出流的数据保存在该字节数组缓冲区中。
        HttpHeaders headers = new HttpHeaders();
        try {
            //防止乱码
            headers.setContentDispositionFormData("attachment", new String("员工表.xls".getBytes("UTF-8"), "ISO-8859-1"));
            headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);//媒体类型信息：二进制流数据(如常见的文件下载）
            workbook.write(baos);//写入数据到流
        } catch (IOException e) {
